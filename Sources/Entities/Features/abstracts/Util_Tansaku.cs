...
*/
 ...
 ");
                ...
                dlgt_CreateJoho(
                    Conv_Taikyokusya.Hanten(ky.Teban),// DoSasite の後なので、相手の手番に進んでいるので、戻すぜ☆（＾～＾）
#if DEBUG
            ...
            )
        {
            ...
            for (int iSs = 0; iSs < AbstractUtilMoveGen.MoveList[fukasa].SslistCount; iSs++)
            {
                ...
                if (Const_Game.SENNITITE_COUNT == ky.Konoteme.GetSennititeCount())
                {
                    ...
                    bool tansakusyaTyakusyu = Util_Tansaku.KaisiTaikyokusya == Conv_Taikyokusya.Hanten(ky.Teban); // 手番はもう相手に回っているので、反転させて、千日手に着手したものかどうか調べるぜ☆
