...
namespace Grayscale.A210_KnowNingen_.B770_Conv_Sasu__.C500____Converter
{
    ...
    public abstract class Conv_StarbetuSasites
    {
            ...
            )
        {
            ...
            foreach (KeyValuePair<Finger, List<MoveEx>> entry1 in komabetuAllMoves.Items)
            {
                ...
                foreach (MoveEx moveA in entry1.Value)// 駒の動ける升
                {
                    if (out_inputMovelist.Contains(moveA.Move))
                    {
                        // 既存の指し手なら無視
                        System.Console.WriteLine("既存の指し手なので無視します1。sfenText=[" + Conv_Move.LogStr_Sfen(moveA.Move) + "]");
                    }
                    else
                    {
                        out_inputMovelist.Add(moveA.Move);
                    }
                }
            }
        }
