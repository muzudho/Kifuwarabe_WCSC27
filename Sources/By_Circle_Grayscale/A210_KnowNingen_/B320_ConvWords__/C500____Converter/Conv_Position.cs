...
namespace Grayscale.A210_KnowNingen_.B320_ConvWords__.C500____Converter
{
    ...
    {
        public static ShogibanImpl ToShogiban(Playerside pside, Position src_Sky, KwLogger logger)
        {
            ...
            foreach (Finger finger in src_Sky.Fingers_All().Items)
            {
                ...
                SyElement masu = Conv_Busstop.GetMasu(busstop);
                Debug.Assert(Conv_Masu.OnAll(Conv_Masu.ToMasuHandle(masu)), "(int)koma.Masu=[" + Conv_Masu.ToMasuHandle(Conv_Busstop.GetMasu(busstop)) + "]");//升番号
        ...
        /// </summary>
        /// <returns></returns>
        public static ulong ToKyokumenHash(Position position)
        {
            ulong hash = 0;
            ...
            foreach (Finger fig in position.Fingers_All().Items)
            {
                ...
                if (
                    Conv_Busstop.GetOkiba(koma)!=Okiba.ShogiBan ||
                    Conv_Busstop.IsKomadai(koma)
                    )
                {
                    // FIXME: 持ち駒はまだ見ていない。
                    continue;
                ...
                ulong value = Util_ZobristHashing.GetValue(
                    Conv_Masu.ToMasuHandle(Conv_Busstop.GetMasu(koma)),
                    Conv_Busstop.GetPlayerside(koma),
        ...
        /// </summary>
        /// <returns></returns>
        public static string LogStr_Description(Position position)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public static string LogStr_Graphical(Playerside pside, Position position, KwLogger logger)
        {
