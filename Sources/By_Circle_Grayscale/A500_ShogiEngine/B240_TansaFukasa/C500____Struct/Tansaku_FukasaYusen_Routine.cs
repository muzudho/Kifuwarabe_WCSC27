    ...
    public class Tansaku_FukasaYusen_Routine
    {
            ...
            )
        {
            ...
            try
            {
                ...
                // 最初は投了からスタートだぜ☆（*＾～＾*）
                result_bestmoveEx = new MoveExImpl(
                    Move.Empty,
                    //最悪点からスタートだぜ☆（＾～＾）
                ...
                if (Tansaku_FukasaYusen_Routine.CanNotNextLoop(yomiDeep, wideCount2, movelist.Count, genjo, args.Shogisasi.TimeManager))
                {
                    ...
                    // 局面に評価を付けます。
                    MoveEx leafMoveEx = Tansaku_FukasaYusen_Routine.Do_Leaf(
                        result_bestmoveEx.Move,
                        genjo,
                        kifu1.GetNextPside(),
                        positionA,
                        args,
        ...
        {
            MoveEx resultMoveEx = new MoveExImpl(
                moveA,
                // 局面に評価値を付けます。
            ...
            )
        {
            ...
            try
            {
                ...
                result_bestmoveEx = new MoveExImpl(
                    ...
                    //Util_Scoreing.GetWorstScore(Conv_Playerside.Reverse( kifu1.GetNextPside()))// プレイヤー1ならmax値、プレイヤー2ならmin値。
                    Util_Scoreing.GetWorstScore(kifu1.GetNextPside())// プレイヤー1ならmax値、プレイヤー2ならmin値。
                    );

                exceptionArea = 2000;

                int wideCount1 = 0;
                foreach (MoveEx iNod_child in movelist2)//次に読む手
                {
                    ...
                    try
                    {
                        ...
                        // 局面
                        {
                            Move tempMove = iNod_child.Move;
                            Util_IttesasuSuperRoutine.DoMove_Super1(
                                Conv_Move.ToPlayerside(iNod_child.Move),
                                ref positionA,//指定局面
                                ref tempMove,
                                kifu1,
                                "C100",
                        ...
                        Util_IttemodosuRoutine.UndoMove(
                            ...
                            iNod_child.Move,//この関数が呼び出されたときの指し手☆（＾～＾）
                            positionA,
                            "C900",
                            logger
                            );
                        ...
                        //----------------------------------------
                        searchedPv[yomiDeep2] = Conv_Move.ToSfen(iNod_child.Move); //FIXME:
                ...
                errH.AppendLine_AddMemo("3. 手(" + node_yomi.Value.ToKyokumenConst.Temezumi + ")読(" + yomiDeep + ") 兄弟最善=[" + a_siblingDecidedValue + "] 子ベスト=[" + a_childrenBest + "] 自点=[" + a_myScore + "]");
                    ...
                    catch (Exception ex)
                    {
                        ...
                        logger.DonimoNaranAkirameta(ex, "棋譜ツリーで例外です(A)。exceptionArea=" + exceptionArea
                            + " entry.Key=" + Conv_Move.ToSfen(iNod_child.Move)
