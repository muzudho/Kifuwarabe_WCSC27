            ...
            else if (GameMode.Game == Util_Application.GameMode)
            {
                // 表示（コンソール・ゲーム用）　局面、あれば勝敗☆（＾～＾）
                {
                    if (Util_Commandline.IsKyokumenEcho)
                    {
                        ...
                        Util_Commands.Sasite_cmd(isSfen, "sasite", ky, syuturyoku);// 参考：指し手表示
                        if (false){
                        ...
                        Util_Commands.Sasite_cmd(isSfen, "sasite seisei", ky, syuturyoku);// 参考：指し手表示 詳細
                        Util_Machine.Flush(syuturyoku);
#endif
...
//#endif
        ...
        public static void DoCommandline(Kyokumen ky, Mojiretu syuturyoku)
        {
            ...
            else if (caret == commandline.IndexOf("position", caret)) { Util_Commands.Position(Option_Application.Optionlist.USI, commandline, ky, syuturyoku); IsKyokumenEcho = false; }
            ...
            else if (caret == commandline.IndexOf("sasite", caret)) { Util_Commands.Sasite_cmd(Option_Application.Optionlist.USI, commandline, ky, syuturyoku); IsKyokumenEcho = false; }
